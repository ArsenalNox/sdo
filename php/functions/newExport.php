...
//Никаких проверок пока что не будет, просто выплёвывает xlsx файл

$sql = 'SELECT * FROM test_results';
$stm = $dtb->query($sql);
if($stm){
    //Указатели на столбец/строку
    $s_column = 1; 
    $s_row = 1;

    //Указаиелт на номера колонок, котрые нужно скрыть
    //Сама таблица 
    ...
    while($row = $stm->fetch(PDO::FETCH_ASSOC)){ //Итерация через все результаты 
        ...
        if($dtb->query($sql)){
            ...
            while($tr_row = $result->fetch(PDO::FETCH_ASSOC)){
            //foreach($dtb->query($sql) as $tr_row){
